...
const makeTransaction =
  require("../../../models/transaction/index").makeTransaction;
const makeUpdateTransaction =
  require("../../../models/transaction/index").makeUpdateTransaction;
...
function listTransactions(httpQuery) {
  ...
  return Transaction.paginate(...paginationParams)
    .then((result) => {
...
function findTransactionBy(prop, val) {
  if (prop === "id") prop = "_id";
  return Transaction.find({ [prop]: val })
    .then((res) => serialize(res[0]))
    .catch(errorFormatter);
}
...
async function addTransactionToWallet(transaction, wallet, month, day) {
  ...
  for (
    let i = startIndex;
    i < getNumberOfDays(wallet.year, new Date().getMonth() + 1);
    i++
  ) {
    if (transaction.transactionType == "Income")
      wallet.data[i] += parseInt(transaction.amount);
    else wallet.data[i] -= parseInt(transaction.amount);
  }
  ...
  console.log(
    `wallet updated, with ${transaction.amount} ${transaction.transactionType}`
  );
}
...
async function updateTransaction(id, updateTransactionInfo) {
  ...
  const validUpdateTransactionData = await makeUpdateTransaction(
  ...
  return Transaction.findByIdAndUpdate(id, updateTransactionInfo, { new: true })
    .then(serialize)
    .catch(errorFormatter);
}
