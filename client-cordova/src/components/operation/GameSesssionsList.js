        ...
        <>
                  ...
                  {columns.map((h) => (
                    <Th
                      ...
                      onClick={() => {
                        if (sort !== undefined) {
                          let newSort = sort;
                          let currentSort = h.accessor + " _id";

                          if (sort === currentSort)
                            newSort = "-" + sort; // to desc
                          else if (sort === "-" + currentSort)
                            newSort = sort.slice(1, sort.length); // to asc
                          else newSort = currentSort; // new sort
                    ...
                    >
                      <Flex alignItems="center">
                        {h.Header}
                        {h.accessor + " _id" === sort && <RiArrowDownSFill size="20" />}
                        {"-" + h.accessor + " _id" === sort && (
                ...
                {gameSessions.map((s) => {
                  return (
                    <Tr key={s.id}>
                      ...
                      >
                        <Flex justify="space-around" gap="0.4rem">
                          <RouterLink to={"/operation/gaming-sessions/" + s.id}>
                            <ActionButton>
                          ...
                          <ActionButton>
                            <FaCashRegister
                              size="20"
                              onClick={async () => {
                                const { updatedGameSession, error } =
                                ...
                                if (error === undefined) {
                                  toast({
                                    title: `CASH ${
                                      updatedGameSession.paid ? "IN" : "REVERT"
                                    }`,
                                    description: `Cashed ${
                                      updatedGameSession.paid ? "IN" : "REVERT"
                                    } Rs.${s.cost}`,
                                    status: updatedGameSession.paid
                                      ? "success"
                                      : "error",
                                    duration: 3000,
                                    isClosable: true,
          ...
          <HStack spacing="1rem">
            {Array.from({ length: pagination.totalPages }, (_, i) => i + 1).map(
              (i) => (
                <Button
                  variant={pagination.page === i ? "solid" : "outline"}
                  colorScheme="teal"
                  key={i}
                  onClick={() => {
